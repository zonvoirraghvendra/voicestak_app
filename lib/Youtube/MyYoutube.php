...
class MyYoutube {
	...
	{
		// Config::set('youtube.client_id', session()->get('youtube.client_id'));
		// Config::set('youtube.client_secret', session()->get('youtube.client_secret'));
	...
	{
		$latest = DB::table('youtube_access_tokens')->where('user_id', Auth::id())
	...
	public function deleteAccessTokenFromDB( $user_id )
	{
		return DB::table('youtube_access_tokens')->where('user_id', $user_id)->delete();
	}
	...
	public function upload($path, array $data, $privacyStatus = 'unlisted')
	{
		...
		#. Set the Filesize
		------------------------------------ */
		$media->setFileSize(filesize(base_path().'/public/uploads/' . $path));

		/* ------------------------------------
		#. Read the file and upload in chunks
		------------------------------------ */
		$status = false;
		$handle = fopen(base_path().'/public/uploads/' . $path, "rb");
