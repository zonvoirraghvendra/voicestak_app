...
class SettingsController extends Controller {
	...
	{
		$ajaxResponse = $this->updateMessage($request->all());
		if($request->ajax()){
			return $ajaxResponse;
		}else{
	...
	private function updateMessage($inputs)
	{
		...
		if(null != $message = $this->messageService->updateMessages( $inputs['id'], $this->request->inputs($campaign_id, $widget_id, $user_id , $name , $inputs['email']) )) {
			...
			if(count($this->youtubeService->isConnected($user_id))){
				...
				// } else {
					try {
						$youtube_id = $this->youtubeService->uploadVideoToYoutube($this->messageService->getMessageById($id)->file_name,$widget->first_name_field_key);
