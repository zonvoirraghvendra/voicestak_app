...
class ApiCampaignsController extends Controller {
	...
	public function index( SingleCampaignRequest $request, CampaignServiceInterface $campaignService, WidgetServiceInterface $widgetService)
	{
		...
		if( $campaignService->collection($campaign_id) )
			return Response::json($campaignService->collection($campaign_id));
	...
	public function getCampaign( $id, SingleCampaignRequest $request, CampaignServiceInterface $campaignService, WidgetServiceInterface $widgetService )
	{
		...
		if( $campaignService->collection($id) )
			return Response::json($campaignService->collection($id));
	...
	{
		return Response::json([ 'campaigns' => $campaignService->getCampaignsList() ]);
	...
	public function store( CampaignServiceInterface $campaignService , CampaignRequest $request , WidgetServiceInterface $widgetService )
	{
		if( $request->get('campaign_id') != '' && null != $widget = $widgetService->createWidget( $request->all() ) ){
			return Response::json(['status' => 'success', 'message' => 'Widget Successfully created.', 'campaign_id' => $request->get('campaign_id'), 'widget_id' => $widget->id]);
			...
			if ( null != $widget = $widgetService->createWidget( array_merge( $request->all() ,  ['campaign_id' => $campaign->id ] ) ) ) {
				return Response::json(['status' => 'success', 'message' => 'Campaign Successfully created.', 'campaign_id' => $campaign->id, 'widget_id' => $widget->id]);
	...
	public function update( $campaign_id , CampaignServiceInterface $campaignService , CampaignRequest $request )
	{
		...
		if( null != $campaignService->updateCampaign( $request->inputs( $campaign_id )) ) {
			return Response::json([ 'status' => 'success', 'campaign_id' => $campaign_id ]);
			...
			if($messageService->destroyCampaignMessages($campaign_id)) {
				if($widgetService->getWidgetsByCampaignID($campaign_id)){
					if($widgetService->destroyCapmaignWidgets($campaign_id) && $campaignService->destroyCampaign($campaign_id)) {
						return Response::json([ 'status' => 'success', 'message' => 'Campaign successfully deleted.' ]);
	...
	{
		return Response::json([ 'campaign' => $campaignService->getCampaignByID( $campaign_id ) ]);
	...
	{
		return Response::json(['widget' => $widgetService->getWidgetByID( $widget_id ), 'campaigns' => $campaignService->getCampaignsList() ]);
	...
	{
		return Response::json([ 'campaign' => $campaignService->getCampaignByID( $campaign_id ), 'widget' => $widgetService->getWidgetByID($widget_id) ]);
		// return view('campaigns.wizard.appearance',[ 'campaign' => $campaignService->getCampaignByID( $campaign_id ), 'widget' => $widgetService->getWidgetByID($widget_id) ]);
	}
	...
	{
		return Response::json([ 'widget' => $widgetService->getWidgetByID( $widget_id ) , 'campaign' => $campaignService->getCampaignByID( $campaign_id ) ]);
		// return view('campaigns.wizard.appearance',[ 'widget' => $widgetService->getWidgetByID( $widget_id ) , 'campaign' => $campaignService->getCampaignByID( $campaign_id ) ]);
	}
	...
	public function postWizardEmbed( $campaign_id , $widget_id , CampaignServiceInterface $campaignService , WidgetEmbedRequest $request)
	{
		//update widget
		// return Response::json([ 'status' => 'success', 'message' => 'Widget successfully updated.' ]);
		return Response::json([ 'status' => 'success', 'message' => 'Campaign has been successfully created.' ]);
