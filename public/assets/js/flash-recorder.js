...
function sendImages()
{
	$.ajax({
		type    : 'POST',
		url     : 'https://app.voicestak.com:4444/images',
		...
		success : function(data){
			...
			if(count < bytes.length) {
				...
				// console.log(temp_bytes);
				$.post("https://app.voicestak.com:4444/images-complete", {token:token, bytes:final_value, finish: true}, function(response){
					makeVideo();
				})
			}
		}
...
function sendAudio() {
	$.ajax({
		type    : 'POST',
		url     : 'https://app.voicestak.com:4444/audio',
		contentType: "application/x-www-form-urlencoded;charset=UTF-8",
		cache  : false,
		...
		success : function(data){
			...
			if(audio_count < audio_bytes.length) {
				...
				$.post("https://app.voicestak.com:4444/audio-complete", {token:token, audio_bytes:audio_final_value, finish: true}, function(response){
					audio_complete = true;
				})
			}
		}
...
function makeVideo(){
	$.ajax({
		type    : 'POST',
		url     : 'https://app.voicestak.com:4444/ffmpeg',
		contentType: "application/x-www-form-urlencoded;charset=UTF-8",
		cache  : false,
		data    : {token : token},
		success: function(data) {
